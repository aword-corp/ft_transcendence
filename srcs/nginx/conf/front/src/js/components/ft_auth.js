...
class FT_Auth extends HTMLElement {
	constructor() {
		...
		var register = `<form class="auth-form">
				<p>
					<label for="id_email">Email:</label>
					<input id="id_email" type=email name="email" maxlength="320" required>
				</p>
				<p>
				...
				<p>
					...
					<input id="id_password_confirmation" type=password name="password_confirmation" required>
				</p>
				<p>
					<label for="id_birth_date">Birth date:</label>
					<input id="id_birth_date" type=date name="birth_date" required>
				</p>
				<p>
		...
		fetch(`https://localhost:8443/api/auth/ft/callback?code=${code}`).then((response) => response.json().then((json) => {
			...
			document.getElementById("id_username").value = json["login"];
